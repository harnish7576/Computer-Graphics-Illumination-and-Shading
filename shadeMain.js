...
function bindVAO (shape, program) {
  ...
  gl.vertexAttribPointer(program.aNormal, 3, gl.FLOAT, false, 0.5, 0.5)
...
function setUpPhong(program) { 
  ...
  let lightingPosition=glMatrix.vec3.fromValues(250,-550,550)
...
function setUpCamera(program) {
    ...
    let projectionT = glMatrix.mat4.create();
    glMatrix.mat4.perspective(projectionT, radians(500), 1, 1, 10)
    gl.uniformMatrix4fv(program.uProjT, false, projectionT);

    // set up your view
    let viewT = glMatrix.mat4.create();
    glMatrix.mat4.lookAt(viewT, [0, 0, 0], [0, 0, 0], [0, 10, 10]);
    gl.uniformMatrix4fv(program.uViewT, false, viewT);
